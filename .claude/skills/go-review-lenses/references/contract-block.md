...
- the subject artifact you are reviewing (a plan, or a change-summary plus the
  change-request evidence that points at the actual diff/branch);
...
If the change-request evidence names a branch/ref, work from the actual code at
that ref (`git diff`/`git show`/reading the files directly) — never review from
the summary alone.
...
- **Correctness lens (`CORR-`):** changed files in full, **plus the boundary
  code they touch** — where external data enters (JSON, subprocess, files,
  env), any goroutines/shared state involved, and the tests/fixtures covering
  the changed code. Follow the change to its edges.
- **Architecture lens (`ARCH-`):** the **package graph around the change**.
  Read the module's `go.mod` to see its package boundaries, then read the
  packages the change touches **and the packages that import them or that they
  import**, following `import` statements outward. If the whole module fits
  your context window, read it; if not, anchor on the changed packages and
  **state explicitly in your output which packages you could not read** — an
...
the subject (and that it returned the subject), and which files/packages you
read to review under this lens. This makes engagement verifiable — a bare
`{"findings":[]}` with no recon entry is treated as a no-op and rejected. If you
did **not** run the query or could not read the subject, say so in `<PREFIX>-0`
and raise the severity accordingly; do not emit an empty clean pass you cannot
back up.
  ...
  "findings": [
    {
      ...
      "severity": "low",
      "category": "recon",
      "description": "Ran `swamp data query 'modelName == \"<factory>\" && name == \"artifact-<workItem>-<subject>\"' --select attributes.payload --json` — returned the subject. Read: <files/packages covered for this lens>.",
      "resolved": false
    },
    {
      "id": "<PREFIX>-1",
      ...
      "description": "Concrete: name the file/line/function, the real-world input or call sequence that breaks it, and how it manifests (silent wrong answer / panic / leak / race).",
      "resolved": false
    }
  ]
}
...
- `category` is optional free text used to group
  (`boundary-validation`, `interface-placement`, `transliteration`,
  `purity-seam`, `dependency-direction`, …).
